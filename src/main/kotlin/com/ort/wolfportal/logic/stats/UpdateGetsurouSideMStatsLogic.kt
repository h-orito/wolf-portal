...
@Component
class UpdateGetsurouSideMStatsLogic : AbstractUpdateStatsLogic() {

    private val logger = LoggerFactory.getLogger(UpdateGetsurouSideMStatsLogic::class.java)
    private val topPageUrl = "http://managarmr.sakura.ne.jp/sow.cgi"
    ...
    override fun fetchVillageInfo(vid: Int): VillageStats? {
        try {
            logger.info("月狼国Side:M vid:${vid} start")
            val village = fetchVillageStatsAndMapping(vid)
            if (village == null) {
                logger.info("月狼国Side:M vid:${vid} エピローグを迎えていません")
            } else {
                logger.info("月狼国Side:M vid:${vid} end")
            }
            return village
        } catch (e: Exception) {
    ...
    private fun fetchEachDayAndMapping(
        ...
        return VillageStats(
            villageName = epiDoc.select("#header h5").text().replace("side:M / エピローグ".toRegex(), ""),
            villageNo = villageNo,
