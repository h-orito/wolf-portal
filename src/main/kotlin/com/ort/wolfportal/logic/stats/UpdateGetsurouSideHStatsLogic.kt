...
@Component
class UpdateGetsurouSideHStatsLogic : AbstractUpdateStatsLogic() {

    private val logger = LoggerFactory.getLogger(UpdateGetsurouSideHStatsLogic::class.java)
    private val topPageUrl = "http://marchocias.xsrv.jp/sow.cgi"
    ...
    override fun fetchVillageInfo(vid: Int): VillageStats? {
        try {
            logger.info("月狼国Side:H vid:${vid} start")
            val village = fetchVillageStatsAndMapping(vid)
            if (village == null) {
                logger.info("月狼国Side:H vid:${vid} エピローグを迎えていません")
            } else {
                logger.info("月狼国Side:H vid:${vid} end")
            }
            return village
        } catch (e: Exception) {
    ...
    private fun fetchEachDayAndMapping(
        ...
        return VillageStats(
            villageName = epiDoc.select("#header h5").text().replace("side:H / エピローグ".toRegex(), ""),
            villageNo = villageNo,
