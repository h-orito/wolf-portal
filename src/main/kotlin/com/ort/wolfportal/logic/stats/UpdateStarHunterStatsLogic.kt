...
@Component
class UpdateStarHunterStatsLogic : AbstractUpdateStatsLogic() {

    private val logger = LoggerFactory.getLogger(UpdateStarHunterStatsLogic::class.java)
    private val topPageUrl = "http://star-hunter.sakura.ne.jp/sow.cgi"
    ...
    override fun fetchVillageInfo(vid: Int): VillageStats? {
        try {
            logger.info("星狩国 vid:${vid} start")
            val village = fetchVillageStatsAndMapping(vid)
            if (village == null) {
                logger.info("星狩国 vid:${vid} エピローグを迎えていません")
            } else {
                logger.info("星狩国 vid:${vid} end")
            }
            return village
        } catch (e: Exception) {
