...
@Component
class UpdateGetsurouSideEStatsLogic : AbstractUpdateStatsLogic() {

    private val logger = LoggerFactory.getLogger(UpdateGetsurouSideEStatsLogic::class.java)
    private val topPageUrl = "http://hrodvitnir.sakura.ne.jp/sow.cgi"
    ...
    override fun fetchVillageInfo(vid: Int): VillageStats? {
        try {
            logger.info("月狼国Side:E vid:${vid} start")
            val village = fetchVillageStatsAndMapping(vid)
            if (village == null) {
                logger.info("月狼国Side:E vid:${vid} エピローグを迎えていません")
            } else {
                logger.info("月狼国Side:E vid:${vid} end")
            }
            return village
        } catch (e: Exception) {
    ...
    private fun fetchEachDayAndMapping(dayLinkMap: Map<String, String>, villageNo: Int, villageUrl: String): VillageStats {
        // エピローグに接続してキャラクター情報を取得
        val epiDoc = BrowserUtil.createConnectionAndConnectWithSleep(topPageUrl + dayLinkMap["エピローグ"] + "&rowall=on").parse()
        ...
        return VillageStats(
            villageName = epiDoc.select("#header h5").text().replace("side:E / エピローグ".toRegex(), ""),
            villageNo = villageNo,
