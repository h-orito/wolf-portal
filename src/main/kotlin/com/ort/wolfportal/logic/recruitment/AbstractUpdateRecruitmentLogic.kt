...
import com.ort.dbflute.exbhv.CountryProgressBhv
import com.ort.dbflute.exbhv.CountryProgressDetailBhv
import com.ort.dbflute.exbhv.CountryRecruitmentBhv
import com.ort.dbflute.exbhv.CountryRecruitmentDetailBhv
import com.ort.dbflute.exentity.Country
import com.ort.dbflute.exentity.CountryProgress
import com.ort.dbflute.exentity.CountryProgressDetail
...
abstract class AbstractUpdateRecruitmentLogic {
    ...
    private fun deleteCountryVillages(countryId: Int?) {
        countryRecruitmentDetailBhv.queryDelete { it.query().queryCountryRecruitment().setCountryId_Equal(countryId) }
        countryRecruitmentBhv.queryDelete { it.query().setCountryId_Equal(countryId) }
        countryProgressDetailBhv.queryDelete { it.query().queryCountryProgress().setCountryId_Equal(countryId) }
    ...
    private fun insertCountryRecruitmentDetail(countryRecruitmentId: Int, entry: Entry<String, String>) {
        val value = entry.value.removeSurrogate() ?: return
        val detail = CountryRecruitmentDetail()
        detail.itemName = entry.key
        detail.itemValue = value
    ...
    private fun insertCountryRecruitment(countryId: Int, villageList: List<CountryVillageDto>) {
        villageList.forEachIndexed { index, village ->
            ...
            village.itemMap.entries.forEach { insertCountryRecruitmentDetail(countryRecruitmentId, it) }
        }
    }
    ...
    fun String.removeSurrogate(): String? {
        val removed = this.replace("[^\\u0000-\\uFFFF]".toRegex(), "").trim()
        return if (removed.isEmpty()) null else removed
    }
}
